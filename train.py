...
if __name__ == '__main__':
    ...
    model_dir = os.path.join(opt.TRAINING.SAVE_DIR, session)
    ...
    if opt.TRAINING.RESUME:
        ...
        if path_chk_rest is None:
            # No UFO checkpoint yet → start from CycleGAN pre-trained weights
            fallback = os.path.join(opt.TRAINING.SAVE_DIR, 'model_C', 'model_best.pth')
            print(f'No {session} checkpoint found. Loading pre-trained CycleGAN weights: {fallback}')
            ckpt = torch.load(fallback, map_location='cuda')
            state_dict = {k.replace('module.', ''): v for k, v in ckpt['state_dict'].items()}
            model_restoration.load_state_dict(state_dict)
            print('Pre-trained weights loaded successfully. Fine-tuning starts from epoch 1.')
        else:
    ...
    for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
        ...
        for i, data in enumerate(tqdm(train_loader), 0):
            ...
            loss_char4 = criterion_edge(restored[0], target4)
            loss_vgg4 = criterion_vgg(restored[0], target4)
            loss_char2 = criterion_edge(restored[1], target2)
            loss_vgg2 = criterion_vgg(restored[1], target2)
            loss_char = criterion_edge(restored[2], target)
            loss_vgg = criterion_vgg(restored[2], target)
            loss4 = loss_char4 + 0.05 * loss_vgg4
            loss2 = loss_char2 + 0.05 * loss_vgg2
            loss = loss_char + 0.05 * loss_vgg

            loss = loss + loss2 + loss4
        ...
        if epoch % opt.TRAINING.VAL_AFTER_EVERY == 0:
            ...
            psnr_val_rgb = utils.torchPSNR(restored[2], target)
